...
//        }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        if collectionView == self.eventsCollectionView {
            let cellSize = CGSize(width: 140, height: self.eventsCollectionView.frame.size.height)
            return cellSize
        }else{
            let cellSize = CGSize(width: 140, height: self.recommendedFriendCollecView.frame.size.height)
